...
bool FanMan::init()
{
	if (!Node::init())
		return false;
	SpriteFrameCache::getInstance()->addSpriteFramesWithFile("FanMan.plist", "FanMan.png");
	_FanManSprite = Sprite::createWithSpriteFrameName("FatMan_stand_1.png");
	this->addChild(_FanManSprite);
	_FanManSprite->setAnchorPoint(Vec2::ANCHOR_BOTTOM_LEFT);
	this->setContentSize(_FanManSprite->getContentSize());
	//this->setAnchorPoint(Vec2::ANCHOR_BOTTOM_LEFT);
	_FanManSprite->setPosition(this->getContentSize() * 0.0f);
	this->setScale(2.0);
